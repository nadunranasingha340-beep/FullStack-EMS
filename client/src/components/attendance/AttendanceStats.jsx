...
const AttendanceStats = ({history}) => {
  ...
  const stats = [
    ...
    {lable: 'Avg. Work Hrs', value: '8.5 Hrs', icon: ClockIcon},
  ]

  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8">
      {stats.map((s) => (
        <div key={s.lable} className="card card-hover p-5 sm:p-6 flex items-center
        gap-4 relative overflow-hidden group">
          ...
          transition-colors duration-200">
            <s.icon className="w-5 h-5 text-slate-600 group-hover:text-indigo-600 transition-colors
            duration-200"/>
          </div>
          <div>
